...
- **Run settlement is owned centrally, not per-provider.** `AgentRunState` (`server/agent/agent-run-state.ts`) tracks every in-flight run — `PendingForegroundRun` or `AutonomousAgentRun` — and is the sole owner of what "settled" means: a run is `settled` only when `AgentRunState` resolves its `settledPromise` (via `settleForegroundRun`/`settleTerminalRun`/`clearAgentRun`), independent of which provider adapter is underneath. `AgentManager`'s own `AgentRunCancellationResult` (`{ status: "not_running" | "settled" | "refused" }`) reads that same settlement, not a provider-specific completion signal. Provider adapters (Claude, Codex, OpenCode, Pi, OMP, ACP) only ever emit `AgentStreamEvent`s (`turn_completed`, `turn_failed`, `turn_canceled`, etc.); they do not and should not decide or track "settled" themselves — that bookkeeping (waiters, `finalizedForegroundTurnIds`, foreground-turn streaming via `ForegroundTurnStream`) lives entirely in `agent-run-state.ts` and is provider-agnostic by design. A plan that adds provider-specific settlement tracking (e.g. inside `providers/pi/agent.ts`) duplicates this contract instead of feeding into it, and risks a second source of truth for whether a run has finished.
