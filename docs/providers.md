...
Pi RPC fire-and-forget extension UI requests are projected by the daemon onto existing portable `tool_call` timeline items with `plain_text` details, so every Paseo client can surface them without Pi-extension-specific client code. Notifications, titles, and editor drafts are one-shot items. Widget updates use stable call ids derived from their extension keys, allowing live clients and projected history to replace earlier values instead of accumulating refresh rows. `setStatus` updates are dropped because they are transient chrome; emitting them mid-turn would break reasoning adjacency and split one reasoning block. Keep Pi's internal marker notifications intercepted before this projection and strip terminal ANSI sequences before persistence.
