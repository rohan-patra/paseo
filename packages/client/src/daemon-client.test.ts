...
test("sendAgentMessage keeps the legacy wire request when no delivery is set", async () => {
  const logger = createMockLogger();
  const mock = createMockTransport();
  ...
  // Legacy void-returning API keeps its exact request shape and return type.
  const sendPromise = client.sendAgentMessage("agent-1", "hello");
  expect(mock.sent).toHaveLength(1);
  const request = parseSentFrame(mock.sent[0]);
  ...
  // A legacy daemon acknowledgement without queue fields is accepted as-is.
  mock.triggerMessage(
    wrapSessionMessage({
  ...
  // The ack-returning API resolves with an empty ack on legacy daemons.
  const ackPromise = client.sendAgentMessageWithAck("agent-1", "hello again");
  expect(mock.sent).toHaveLength(2);
  const ackRequest = parseSentFrame(mock.sent[1]);
  expect("delivery" in ackRequest).toBe(false);
  mock.triggerMessage(
    wrapSessionMessage({
      type: "send_agent_message_response",
      payload: {
...
test("sendAgentMessage forwards explicit delivery and returns the queue acknowledgement", async () => {
  const logger = createMockLogger();
  const mock = createMockTransport();
  ...
  const connectPromise = client.connect();
  mock.triggerOpen({ features: { agentMessageQueue: true } });
  await connectPromise;
  expect(client.getLastServerInfoMessage()?.features?.agentMessageQueue).toBe(true);

  const sendPromise = client.sendAgentMessageWithAck("agent-1", "queue this", {
    delivery: "follow_up",
  });
  expect(mock.sent).toHaveLength(1);
  const request = parseSentFrame(mock.sent[0]);
  expect(request.type).toBe("send_agent_message_request");
  expect(request.delivery).toBe("follow_up");
